...
多条链接时，下载/转码/封面可以同时进行；DeepL 翻译和 B 站上传会串行，避免限流。Windows 上 YouTube 下载同一时刻只有 1 路（防文件占用错误），但**上传 B 站时其它任务仍会继续下载**。其中一条失败不会中断其它条。已经投稿成功的视频在批量模式下会跳过（单条仍会报错，可用 `--force` 重做）。
