...
建议默认：连接超时 3 秒，本地阶段硬预算 300 秒，DeepL 阶段硬预算 90 秒，整组预算 420 秒。首选从取得翻译执行槽开始计时，预算包含加载、校验和该 Provider 的重试；排队另行记录。总预算是硬上限，剩余额度不足则提前停止，不把每个 HTTP 的 timeout 当作总 timeout。较长的本地预算用于覆盖 4 GB 显存设备上 CPU/GPU 混合推理的冷启动与吞吐波动；取消仍应立即终止隔离进程。
